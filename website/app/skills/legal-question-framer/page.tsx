...
export default function LegalQuestionFramerSkill() {
  const skill = getSkill("legal-question-framer");
  return (
    <>
      <SkillBreadcrumb slug="legal-question-framer" />
      <p className="eyebrow">SLS Legal Skills &middot; {skill.status} &middot; v{skill.version}</p>
      <h1>{skill.name}</h1>
      <p className="lede">Frame a precise legal question before researching or teaching it.</p>
      ...
      <section className="grid two">
        <article className="card">
          <h2>What it does</h2>
          <p>It separates a research question from a proposed answer and pins down jurisdiction, time frame, procedural posture, legal roles, the material facts, disputed assumptions, the governing source hierarchy, and the subquestions that actually need answering.</p>
        </article>
        <article className="card">
